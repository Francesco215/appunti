...
\begin{document}
	...
	\chapter{Algebra lineare}
		...
		\section{Trasformata di Furiè}
			...
			Visto che le funzioni trigonometriche sono ortogonali tra di loro in un intervallo finito, vogliamo vedere questa proprietà è anche vera su tutti i reali, di conseguenza ipotizziamo che $F^{-1}(k,x)t=\alpha e^{ikx}$ dove $\alpha \in \rm I\!\!\! C$.
			Adesso vediamo le proprietà del prodotto di queste matrici e vediamo che riusciamo a tirarci fuori.
			\begin{equation}
				\hat I(k,\overline k)=F^{-1}F=\alpha\int_{-\infty}^{\infty}e^{ikx}e^{i\overline k x}dx=\alpha\int_{-\infty}^{\infty}e^{i(k+\overline k)x}dx
			\end{equation}
			Segue immediatamente che $\hat I(\lambda k,\lambda \overline k)=\frac{\hat I(k,\overline k)}{\lambda}$ e che $\hat I(k+\lambda,\overline k-\lambda)=\hat I(k,\overline k)$. Ciò implica che la funzione è costante lungo le rette parallele alla bisettrice del secondo e quarto quadrante e che lungo le rette parallele alla bisettrice del primo e terzo quadrante $\hat I$ va come $1/x$.\newline
			Questo è molto importante perchè significa che sapere quanto vale la funzione in un solo punto\footnote{che non sia lungo la bisettrice del secondo e quarto quadrante (c'è il rischio che diverga)} significa sapere come vale la funzione in tutto $\rm I\! R^2-span\{(1,-1)\}$.\newline
			Dalla serie di Fourier sappiamo che
			\[
				\pscal{\vettore e_1}{\vettore e_2}=0=\int_{-n\pi}^{n\pi}e^{-ix}e^{2ix}dx\quad \quad \forall n\in \rm I\! N
			\]
			Visto che la relazione di prima vale per ogni $n$ naturale, varrà anche se mando n a infinito. Quindi $\hat I(-1,2)=0$ e per le relazioni di prima si ha che $\hat I$ è nullo in tutto il piano cartesiano tranne la bisettrice del secondo e quardo quadrante. per sapere quanto vale la basta sostituire a $\overline k$ $k$, infatti
			\[
				\hat I(k,k)=\int_{-\infty}^{+\infty}e^{i(k-k)x} dx=\int_{-\infty}^{+\infty}1dx=+\infty
			\]
			quindi $\hat I(k,-\overline k)$è proporzionale alla matrice identità, e $F^{-1}(k,x)$ è proporzionale a $e^{-ikx}$.
